...
//  digitalWrite(TOGGLEPIN, toggle);
  ...
  if (params[MODE] == 0) {
      // Position mode
      X  = (((long) params[K0]) << OVERSAMPLE) + 10*params[K1]*(delWBA - params[K2]);
  } else if (params[MODE] == 1) {
      // Velocity mode
      dXdT = params[K3] + params[K4]*(delWBA - params[K5]);
      X = X + dXdT; 
  } else if (params[MODE] == 2) {
      // Velocity mode for drum as X, but valves trigger in position mode as sX and output as sX
      sX  = (((long) params[K0]) << OVERSAMPLE) + 10*params[K1]*(delWBA - params[K2]);
  ...
  // Position wrap around for sX
  if ((sX >= topLimitX) && (params[MODE] == 3)) {
      sX -= (topLimitX - bottomLimitX);           
  } else if (X < bottomLimitX) { 
      sX += (topLimitX - bottomLimitX); 
  } 
  ...
  if (displayOn) {
    ...
    if ((params[CENTERMODE] == 5) ||  (params[CENTERMODE] == 6) ||  (params[CENTERMODE] == 7)) {
      ...
      tempCoord = (byte) (sX >> OVERSAMPLE);
      tempCoord = tempCoord + 8;
    } else {
    ...
    // Check for latch activation
    if (((1 << laserValveCoord) & params[LATCHARMED]) || latchFired) {
      // First, make sure latch will remain active
      // nb: latchFired is reset whenever params[LATCHARMED] is re-written.
      latchFired = true;

      // Then, set laser or olfactometers to ON or OFF depending on LATCHSTATE
      if ((1 << 0) & params[LATCHSTATE]) {
        params[SECTORSARMED] = 0xFFFF;
      } else {
        params[SECTORSARMED] = 0x0000;
      }
      if ((1 << 1) & params[LATCHSTATE]) {
        params[OLF1ARMED] = 0xFFFF;
      } else {
        params[OLF1ARMED] = 0x0000;
      }
      if ((1 << 2) & params[LATCHSTATE]) {
        params[OLF2ARMED] = 0xFFFF;
      } else {
        params[OLF2ARMED] = 0x0000;
      }
      if ((1 << 3) & params[LATCHSTATE]) {
        params[OLF3ARMED] = 0xFFFF;
      } else {
        params[OLF3ARMED] = 0x0000;
      }  
    }
  ...
  // Write the output to the analog pin
   if (params[CENTERMODE] == 7) {
     ...
     scaledCoords = ((sX >> OVERSAMPLE) - MINX)*255/(MAXX - MINX);
   } else {
