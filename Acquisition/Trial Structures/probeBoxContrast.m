...
for epoch=1:7
    % Determine when each contrast happens
    contrastN = contrastSequence(epoch);
    contrast = contrastList(contrastN);
    boxStart = 4*min + 5 + (4*min+10)*(epoch-1);
    boxEnd   = boxStart + 2*min;
    stripeStart = boxEnd + 5;
    stripeEnd   = stripeStart + 2*min;
    
    boxSet = ['ardSetBox(8,8,21); ardSetColors(0,',num2str(contrast),');'];
    stripeSet = ['ardSetBox(8,32,1);ardSetColors(0,',num2str(contrast),');'];
    ...
    histogramBounds(2+2*(contrastN-1),:) = [boxStart,boxEnd];
    % Histogram time for stripe after box
    histogramBounds(3+2*(contrastN-1),:) = [stripeStart,stripeEnd];
end
