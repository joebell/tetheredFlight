...
latchStateSafe = 'ardSetLatchArmed(''0000'');ardSetLatchState(false,false,false,false);';
latchStateBack = 'ardSetLatchArmed(''0000'');ardSetLatchState(false,false,false,false);';
...
for i=20:92
    ...
    if randi(2) > 1
        ...
        % No-Odor behind Trial
                                        %--Time------Visual-------Laser-----Odor1----Odor2----Odor3                                   
        trialStructureList(2+(i-1)*4,:) = {randTime,  visStimLock, laserOff, odorOff,odorOff,odorOff,latchOff};
        trialStructureList(3+(i-1)*4,:) = {unlockTime,   visStimN, laserOff, odorOff,odorOff,odorOff,latchStateBack};
        trialStructureList(4+(i-1)*4,:) = {odorTime,     visStimN, laserOff, odor1OnR,odor2OnR,odor3OnR,latchArmed};
        trialStructureList(5+(i-1)*4,:) = {testTime,     visStimN, laserOnR, odor1OnR,odor2OnR,odor3OnR,''};
