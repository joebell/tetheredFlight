...
latchArmed = 'ardSetLatchArmed(''ff00'');';
...
for i=1:72
    ...
    rando = randi(3);
    if (rando == 1)
        % Odor in front training Trial
                                        %--Time------Visual-------Laser-----Odor1----Odor2----Odor3                                   
        trialStructureList(2+(i-1)*4,:) = {randTime,  visStimLock, laserOff, odorOff,odorOff,odorOff,latchOff};
        trialStructureList(3+(i-1)*4,:) = {unlockTime,   visStimN, laserOff, odorOff,odorOff,odorOff,latchStateSafe};
        trialStructureList(4+(i-1)*4,:) = {odorTime,     visStimN, laserOnF, odor1OnF,odor2OnF,odor3OnF,latchArmed};
    ...
    elseif (rando == 2)
        ...
        % Histogram during whole epoch
        histogramBounds(13, (2*i - 1)) = randTime;
        histogramBounds(13, (2*i - 0)) = endTime;
        % Histogram during odor
        histogramBounds(14, (2*i - 1)) = odorTime;
        histogramBounds(14, (2*i - 0)) = testTime;
        % Histogram during test
        histogramBounds(15, (2*i - 1)) = testTime;
        histogramBounds(15, (2*i - 0)) = endTime;    
    end
end
