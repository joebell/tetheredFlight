...
% xlim([rangeT(1) rangeT(end)]);
% ylim([rangeX(1) rangeX(end)]);
% title('Box + Test Odor P(angle|time)');
% xlabel('Time');
% set(gca,'YTick',[90 180 270 360]);
% ylabel('Angle');
% 
...
xlabel('Angle (deg)');
ylabel('Speed (deg/sec)');
title('Box + Test Odor P(angle,speed)');
plot2dFormat();
...
xlabel('Angle (deg)');
ylabel('Speed (deg/sec)');
title('Box + Test Odor <dWBA/dt(angle,speed)>');
plot2dFormat();
