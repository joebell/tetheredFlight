...
for histN = 1:numHist
    ...
    n{histN} = zeros(size(rangeT,2),size(rangeX,2));
...
%             
%             figure(traceFig);
%             subplot(numHist,1,histN); hold on;
...
%             set(gca,'YTick',[90 180 270 360]);
            ...
            subplot(6,numHist,(dataSource-1)*numHist + histN); hold on;
            plot(rangeX,sum(thisHist,1).*size(fileList,2),'b');
            n{histN} = n{histN} + thisHist;
...
%     figure(histFig);
%     subplot(numHist,1,histN); hold on;
%     h = pcolor(rangeT,rangeX, (n{histN})');
