function out = analyzeWBAStats(filename, comment)

%% Parameters

freqThresh = 160;       % Discard WBA's if Freq is below this.
...
% WBAL/R hist, wba L-R hist, wba L+R hist, freq hist, freq vs. L+R scatter
numPlots = 5;

for histN = 1:numHist
    plotNbase = (histN-1)*numPlots;
    ...
    subplot(numHist,numPlots, plotNbase + 1);
    LN = hist(data.LAmp, singleRange);
    RN = hist(data.RAmp, singleRange);
    plot(singleRange,LN,'b'); hold on;
    plot(singleRange,RN,'r');
    set(gca,'YTick',[]);
    if histN == numHist
        xlabel('WBA L & R');
    end
    ...
    subplot(numHist,numPlots,plotNbase + 2);
    diffN = hist(data.LAmp(startSample:endSample) - data.RAmp(startSample:endSample),...
        diffRange);
    plot(diffRange,diffN,'b');
    set(gca,'YTick',[]);
    if histN == numHist
        xlabel('WBA L - R');
    end
    ...
    subplot(numHist,numPlots,plotNbase + 3);
    sumN = hist(data.LAmp(startSample:endSample) + data.RAmp(startSample:endSample),...
        sumRange);
    plot(sumRange,sumN,'b');
    set(gca,'YTick',[]);
    if histN == numHist
        xlabel('WBA L + R');
    end
    ...
    subplot(numHist,numPlots,plotNbase + 4);
    sumN = hist(data.Freq(startSample:endSample), freqRange);
    plot(freqRange,sumN,'b');
        set(gca,'YTick',[]);
    if histN == numHist
        xlabel('Freq (Hz)');
    end
    ...
    subplot(numHist,numPlots,plotNbase + 5);
    amps = data.LAmp(startSample:endSample) + data.RAmp(startSample:endSample);
    freqs = data.Freq(startSample:endSample);
    n = hist3([amps,freqs],{sumRange,freqRange});
    colormap(jet);
    h = pcolor(sumRange,freqRange,log(n'));
    set(h,'EdgeColor','none');
    xlabel('WBA Sum');
    ylabel('Freq (Hz)');
    
    
    %scatter(data.LAmp(startSample:endSample) + data.RAmp(startSample:endSample),data.Freq(startSample:endSample),1);
    %xlim([50 500]);
    %ylim([150 250]);
